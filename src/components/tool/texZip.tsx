...
export const TexZip = () => {
  ...
  const handlePasteZip = useCallback(() => {
    ...
    if (textArea && resultArea) {
      ...
      const shortedEntries = makeShorter(filteredEntries, { zipMode: true });
  ...
  useEffect(() => {
    ...
    if (button) {
      button.addEventListener("click", handlePasteZip);
    }
    if (textArea) {
      textArea.addEventListener("input", handlePasteZip);
    }

    return () => {
      if (button) {
        button.removeEventListener("click", handlePasteZip);
      }
      if (textArea) {
        textArea.removeEventListener("input", handlePasteZip);
      }
    };
  }, [handlePasteZip]);
